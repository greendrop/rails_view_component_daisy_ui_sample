...
RSpec.describe AdminSite::DatetimeFieldComponent, type: :component do
  describe 'snapshot' do
    let!(:snapshot_prefix) { described_class.name.underscore }
    let(:base_args) { { name: 'published_at' } }
    ...
    context 'valueを指定' do
      let!(:instance) { described_class.new(**base_args, value: '2025-01-01 12:00:00') }
      let!(:rendered) { render_inline(instance) }
