...
RSpec.describe AdminSite::PasswordFieldComponent, type: :component do
  describe 'snapshot' do
    let!(:snapshot_prefix) { described_class.name.underscore }
    let(:base_args) { { name: 'username' } }
    ...
    context 'valueを指定' do
      let!(:instance) { described_class.new(**base_args, value: 'testuser') }
      let!(:rendered) { render_inline(instance) }
    ...
    context 'placeholderを指定' do
      let!(:instance) { described_class.new(**base_args, placeholder: 'Enter your name') }
      let!(:rendered) { render_inline(instance) }
