...
RSpec.describe AdminSite::NumberFieldComponent, type: :component do
  describe 'snapshot' do
    let!(:snapshot_prefix) { described_class.name.underscore }
    let(:base_args) { { name: 'age' } }
    ...
    context 'valueを指定' do
      let!(:instance) { described_class.new(**base_args, value: '1') }
      let!(:rendered) { render_inline(instance) }
      let!(:snapshot_name) { File.join(snapshot_prefix, 'value') }
    ...
    context 'placeholderを指定' do
      let!(:instance) { described_class.new(**base_args, placeholder: 'Enter your email') }
      let!(:rendered) { render_inline(instance) }
      let!(:snapshot_name) { File.join(snapshot_prefix, 'placeholder') }
    ...
    context 'min, max, stepを指定' do
      let!(:instance) { described_class.new(**base_args, min: 0, max: 100, step: 2) }
      let!(:rendered) { render_inline(instance) }
      let!(:snapshot_name) { File.join(snapshot_prefix, 'min_max_step') }
